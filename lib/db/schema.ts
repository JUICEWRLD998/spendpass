...
export const product = pgTable(
  "product",
  {
    ...
    merchant: text("merchant").notNull().default("spendpass-store"),
  ...
  (table) => [
    ...
    index("product_merchant_idx").on(table.merchant),
    index("product_priceCents_idx").on(table.priceCents),
  ],
);
...
export const cartItem = pgTable(
  ...
  (table) => [
    ...
    index("cartItem_agentId_idx").on(table.agentId),
    index("cartItem_userId_agentId_idx").on(table.userId, table.agentId),
  ],
);
...
export const order = pgTable(
  ...
  (table) => [
    ...
    index("order_agentId_idx").on(table.agentId),
  ],
);
...
export const agentClientKv = pgTable("agent_client_kv", {
  key: text("key").primaryKey(),
  value: text("value").notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at")
