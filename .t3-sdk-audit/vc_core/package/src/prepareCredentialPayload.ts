import { MANDATORY_CONTEXT_URI } from './constants';
import { DID } from './didManager';
import { Uri, VerificationOptions } from './types';
 ...
 */
export async function prepareCredentialPayload(
  ...
  const id = uuidv4();
  credentialPayload.id = ('urn:uuid:' + id) as Uri;

  if (
    options &&
    options?.revocationRegistryAddress &&
    options?.provider &&
    options?.didRegistryAddress
  ) {
    credentialPayload.credentialStatus = {
      type: 'T3RevocationRegistry',
      chain_id: String((await options.provider.getNetwork()).chainId),
