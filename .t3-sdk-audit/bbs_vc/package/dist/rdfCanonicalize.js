...
exports.canonicalizeAndGroup = canonicalizeAndGroup;
exports.stripBlankNodePrefixes = stripBlankNodePrefixes;
exports.selectJsonLd = selectJsonLd;
exports.skolemizeExpandedJsonLd = skolemizeExpandedJsonLd;
exports.relabelBlankNodes = relabelBlankNodes;
exports.jsonPointerToPaths = jsonPointerToPaths;
exports.createShuffledIdLabelMapFunction = createShuffledIdLabelMapFunction;
 ...
 */
function canonicalizeAndGroup(document, labelMapFactoryFunction, groupDefinitions, options) {
    return __awaiter(this, void 0, void 0, function* () {
        const expanded = yield jsonld_1.default.expand(document, {
            // TODO: true
            safe: true,
            documentLoader: options.documentLoader,
        });
        const skolemized = {};
        skolemized.expanded = skolemizeExpandedJsonLd(expanded, {
            bnPrefix: 'urn:bnid:',
        });
        if (!document['@context']) {
            throw new Error('Document must have a context');
        }
        skolemized.compact = yield jsonld_1.default.compact(skolemized.expanded, document['@context'], {
            // TODO: true
            safe: true,
            documentLoader: options.documentLoader,
        });
        ...
        const deskolemizedNQuads = yield toDeskolemizedNQuads(skolemized.expanded, options);
        /*
        ...
        const canonicalNQuads = yield jsonld_1.default.normalize(deskolemizedNQuads.join(''), {
            algorithm: 'URDNA2015',
        ...
        for (const name in groupDefinitions) {
            ...
            const selectTemp = yield selectCanonicalNQuads(pointers, skolemized.compact, labelMap, options);
 ...
 * labels (nquads).
 */
function selectCanonicalNQuads(pointers, skolemizedCompactDocument, labelMap, options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const deskolemizedNQuads = yield toDeskolemizedNQuads(selectionDocument, options);
 ...
 */
function selectJsonLd(document, pointers) {
    ...
    selectionDocument['@context'] = (0, klona_1.klona)(document['@context']);
...
function skolemizeExpandedJsonLd(expanded, options = {}) {
    ...
    expanded.forEach((element) => {
        ...
        if (typeof element !== 'object' ||
            ...
            skolemizedExpandedDocument.push((0, klona_1.klona)(element));
        }
        else {
...
// helper function
function toDeskolemizedNQuads(skolemized, options) {
    return __awaiter(this, void 0, void 0, function* () {
        // Convert skolemized doc to RDF to produce skolemized N-Quads.
        const rdfOptions = Object.assign(Object.assign({ 
            // TODO: true
            safe: true }, options), { format: 'application/n-quads' });
        let rdf; // silly declaration because RdfOrString type is not exported in the jsonld lib
        if (Object.keys(skolemized).length === 0) {
            rdf = '';
        }
        else {
            rdf = yield jsonld_1.default.toRDF(skolemized, rdfOptions);
        }
...
function relabelBlankNodes(nquads, labelMap) {
    ...
    const replacer = (_m, _s1, label) => '_:' + labelMap.get(label);
...
function selectPaths(paths, document, selectionDocument, arrays) {
    ...
    if (typeof value !== 'object' || value === null) {
        ...
        if (Array.isArray(value)) {
            selectedValue = (0, klona_1.klona)(value);
        }
        else {
            ...
            selectedValue = Object.assign(Object.assign({}, selectedValue), (0, klona_1.klona)(value));
        }
    }
...
function createShuffledIdLabelMapFunction(hmac) {
    return function labelMapFactoryFunction(canonicalIdMap) {
        ...
        for (const [input, c14nLabel] of canonicalIdMap) {
            ...
            bnodeIdMap.set(input, `u${base64url_1.default.encode(Buffer.from(hashed))}`);
        }
...
function createHmac(key) {
    ...
    return function hmacFunc(input) {
        return (0, hmac_1.hmac)(sha256_1.sha256, key, input);
    };
}
function randomBytes(bytesLength = 32) {
    return new Uint8Array(crypto_1.default.randomBytes(bytesLength).buffer);
}
//# sourceMappingURL=rdfCanonicalize.js.map